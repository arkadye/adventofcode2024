...
#define ADVENT_A_STAR_USE_UTILS_SORTED_VECTOR 0
...
#define ADVENT_A_STAR_USE_UTILS_SORTED_VECTOR 0
		...
		using NodeSet = utils::small_vector<ValueType, Sorter>;
		template <typename ValueType, typename Sorter>
			...
			std::size_t estimated_number_of_nodes = 1)
	{
		...
		while (!nodes_to_search.empty())
		{
			...
			for (auto& n : next_nodes)
			{
				...
				std::cout << "\nAdding node [ID:" << new_it->id <<  ";C:" << new_it->cost << ";H:" << new_it->with_heuristic << "] " << new_it->node << " to check.";
			}
